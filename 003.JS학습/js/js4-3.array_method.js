...
// 0. 기본정보 셋팅  //////////////////
// (1) 배열변수 선언과 할당 -> 배열 리터럴방식
const fruit = ["배", "사과", "용과", "딸기"];

// (2) 과일명과 배경이미지명을 매칭함 -> 객체 리터럴방식
const frObj = {
  배: "fruits_01",
  사과: "fruits_02",
  용과: "fruits_14",
...
// let newArr= arrFruits.map(v=>`<option>${v}</option>`);
// -> 화살표함수에서 중괄호를 사용하면 return할 경우 return 키워드를 써줘야함
// 따라서 중괄호 생략시 리턴키워드 자동생략 문법을 사용하면 간단히 표현할 수 있다
...
// 대상 : #anum -> aNum변수
// 데이터 : fruit 배열
// 갱신시 계속 재바인딩 되어야 함 (함수화 필요),{}사용하면 리턴값을 설정해줘야하므로 생략함
...
function showFruit() {
  ...
  if (btxt === "과일주세요~!") {
    ...
    fruit.forEach((v) => {
      hcode += `
          <li style="background:url(./addimg/${frObj[v]}.png) no-repeat center/cover">${v}
          </li>
        `;
    }); /////// foreach /////
    hcode += `</ul>`;
  ...
  // (6) '중간배열삭제!' 버튼 : splice()
  // 삭제시 : splice(순번) -> 해당순번부터 뒤를 모두 삭제
  // splice(순번, 개수) -> 해당순번부터 개수만큼 삭제
  else if (btxt === "중간배열삭제") {
    // 지울 순번 읽어오기 (대상: #anum -> #aNum 변수)
