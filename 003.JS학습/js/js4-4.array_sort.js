...
/****************************************************** 
  ...
  *************************************************************
      ...
      1. find() 메서드 : 찾으면 저장 / 함수 구성이 for each문과 map()메서드랑 같음
...
function changeSort(e,arrObj){ 
  ...
  //#해석 : a랑 b가 같으면? 아무것도 안함 , b가 a보다 크면 순서유지, a가 b보다 크면 순서바꿔


    // [공통 정렬 처리하기] - 크기는 정렬 순서
    // 문자든 숫자든 sort()메서드의 내부적 처리에서 앞뒤 문자가 같으면 0, 
    // 뒷문자가 크면 -1, 뒷 문자가 작으면 1로 리턴값이 처리하면 된다
    // -> [[숫자 시그널]] - 0 : 아무것도 안함, 1 : 순서바꿔서 유지, -1 : 순서 유지
    // -> [내부적 처리란?] 문자일 경우: '가'>'나' -> 1로 처리 
  ...
  // ((주의!!)) 원본배열을 정렬후엔 원본배열은 없어진다 -> 주소카피 현상
  // 원본 배열을 다른 변수에 할당 후 다른 변수를 정렬해도 여전히 원본배열은 없어진다
