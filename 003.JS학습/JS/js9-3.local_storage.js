...
/*************************************************************** 
    ...
    1. 정의 : 브라우저별 로컬 어플리케이션 영역에 저장되는 
              ✔스트링데이터✔ 저장소(JS API)

    2. 유지 : 같은 PC, 같은 브라우저(재설치없이사용) 일 경우 계속유지됨
    (단, 같은파일일 지라도 여는 경로에 따라 같은 변수도 따로 관리된다! - 기준이 도메인경로/주소)
...
// 게시판 최초 호출 : 로컬쓰 minfo 존재여부에 따라 처리
...
console.log("idx값 배열:", localData.map(v=>v.idx));
    ...
    // 3. 입력할 데이터 객체 형식으로 배열에 넣기 : 배열.push({객체})
    localData.push({
      // 순번은 배열객체 idx 값 중 최대값을 구하여 1 더한다
...
mFn.qs("#mobtn").onclick = () => {
  ...
  localData.find(v=>{
    ...
    if(v.idx == optVal){
      ...
      // 변수에 find 할당시 저장하거나 여기서 순회를 끝낸다는 의미
      return true
    };
  }); //// find /////
