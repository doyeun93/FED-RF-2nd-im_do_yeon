...
// 4. 데이터글자 띄어쓰기를 기준으로 글자묶음으로 태그로 싸기
// 기준 문자열로 문자를 잘라서(split = 문자가공) 배열로 만들기
// -> 문자열.split(자를기준문자)
// 결과 : 자를 문자열은 없어지고 배열로 나눠져 저장
const newText = myText.split(' '); // '' 사이에 공백(스페이스) 기준으로 나눔
// console.log(newText); // myText의 단어들이 호출
...
// 6. 스크롤 이벤트 발생시 글자박스가 화면 1/2 위치에서 등장할 수 있도록 클래스 on주기
