...
// 메인 출력 컴포넌트 //////////////
// 컴포넌트 내부에서 호출하는 다른 컴포넌트를 보통 서브 컴포넌트라고 부른다
...
// 먼저 가상돔에 컴포넌트 리턴코드를 넣어준다
ReactDOM.render(
  <Developer isNormal={true} isrc={devImg[1]} ialt="일반인 동석" itit="개발자가 뭡니까?" />,
  root[1]
);
...
// 음식리스트
const foods = ["스파게티", "라면", "짜글이", "짜장면", "마라탕"];
...
function WishList({ wList }) {
  ...
  return (
    <React.Fragment>
      ...
      {/* 배열값이 0인 경우 다른것 출력하기 */}
      {wList.length == 0 && <h2>아직 개발자 음식 리스트가 업데이트 되지 않았습니다.</h2>}
    </React.Fragment>
  );
} /////// WishList 컴포넌트 /////////
    ...
    <React.Fragment>
      ...
      {wList.length > 0 && (
        <div>
          ...
          {wList.map((x) => (
            <MakeImage isrc={x.poster} ialt={x.mtit} icss={{ width: "100px" }} />
          ))}
        </div>
      )}
...
// 4-2. 전체 출력 컴포넌트
// 구성 : 전체 타이틀(SetTitle 컴포넌트) + 변경버튼 + 작가 타이틀과 그림 출력(MakePainting 컴포넌트)
// 특이사항 : 변경 버튼 클릭시 MakePainting 컴포넌트의 데이터를 변경하여 다시 출력하도록 함(Hook사용)

function ShowLikePinter({ isChange }) {
  // isChange - 기존 작가를 변경하는 여부 전달변수 / true(change) or false(not change) -> 삼항연산자 사용
...
/********************************************************* 
    ...
    2. 구현방법:
        ...
        2) 코딩법 : useState() 메서드사용
           ...
           -> set변수명(값) : 메서드형태로 후크변수의 값을 셋팅함!(업데이트 함수)

        3) 작동원리 
            - useState에 쓴 초기값이 배열변수 첫번째 변수에 할당된다!
