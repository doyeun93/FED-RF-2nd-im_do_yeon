...
// props는 컴포넌트 호출시 속성 값을 객체로 전달하는 것을
// 받아주는 변수다. (변수명은 자유)
// 사용법 : props.속성명
// 주의 : 속성명은 컴포넌트를 호출하는 곳에서 정해진 이름을 똑같이 사용
// 명시적으로 변수명을 사용하려면 구조분해할당 형식을 사용하면 된다
// => {변수명,변수명,....}
// 구조분해할당으로 받을 경우의 장점은 특정 속성명을 지정하는 효과가 있고
// 몇 개의 어떤 속성을 받는지 개발자가 명시적으로 알 수 있다
...
function IntroCar({ brand, modelNum }) {
  ...
  return (
    <React.Fragment>
      ...
      {/* 추가 질문 컴포넌트 호출 */}
      <AskMoreInfo 
      brand={brand} model={setInfo.model} 
      color={setInfo.color} opt={setInfo.opt} />
    </React.Fragment>
  );
} ////// IntroCar 컴포넌트 //////////////////
...
/* 
    ...
    2. 상세 모델 배열 번호 : 선택 객체 속성값 하위 배열 번호 
...
function DetailCarInfo({ brand, model, color, opt }) {
  ...
  // 전달속성은 model(모델명), color(차 색), opt(css 옵션)
  ...
  return (
    <React.Fragment>
      ...
      {/* 리액트 style 속성의 값으로 객체를 CSS속성에 맞게 주면 
        인라인 코드로 CSS를 세팅할 수 있다 */}
    </React.Fragment>
  );
} //////  DetailCarInfo 컴포넌트 ///////////
...
ReactDOM.render(
  <div>
    <ShowBrandCar brand="기아 레이" modelNum={0} />
    <ShowBrandCar brand="현대 제네시스" modelNum={0} />
    <ShowBrandCar brand="기아 레이" modelNum={1} />
    <ShowBrandCar brand="현대 제네시스" modelNum={1} />
    <ShowBrandCar brand="기아 레이" modelNum={2} />
    <ShowBrandCar brand="현대 제네시스" modelNum={2} />
  </div>,
  mFn.qs("#root1")
);
