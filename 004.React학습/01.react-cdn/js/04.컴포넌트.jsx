...
// 리액트에서는 from 뒤 파일명에 확장자 "js/ jsx"를 생략해도 됨
...
/************************************************* 
    ...
    [ 첫번째 컴포넌트 만들기! ]
    - 리액트 컴포넌트 이름은 반드시 ★첫글자가 대문자★로 만든다!
    (안지키면 적용안됨!!!)
...
class GoghWork extends React.Component {
  // 클래스형 컴포넌트에서는 render(){} 메서드로 HTML코드를 리턴한다
  // 내부에 return구문 필요
  render() {
    // HTML 코드 리턴
    return (
...
/************************************************* 
    ...
    -> 컴포넌트에 {}를 사용하면 개별적인 속성변수를 
    전달받을 수 있다(구조 분해 할당 방식)
*************************************************/
...
// 좋아하는 색과 음식, 취미를 각각 속성명으로 생성하여 컴포넌트를 
// 호출하면 개별적으로 속성을 구분할 수 있다
// 출력: 세번째 .root
ReactDOM.render(<FavoriteThings color="lightpink" food="라면" hobby="베이킹" />,target[2]);
// FavoriteThings을 대문자가 아닌 소문자쓰면 에러남

// 출력: 네번째 .root
ReactDOM.render(<FavoriteThings color="파스텔톤" food="짜장면" hobby="운동" />,target[3]);
