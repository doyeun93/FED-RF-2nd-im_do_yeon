...
-> current를 써야하는 이유는 useRef컴포넌트 객체에서 이 변수를 관리해주고 값을 
유지 업데이트 하는데 사용하는 속성으로 current를 사용하도록 지정함
...
// [4] 컴포넌트 외부 일반 변수
// 1. 전역변수 역할을 하지만 컴포넌트가 여러개 사용될 때 이 변수를 업데이트하면
// 여러가지 프로그래밍이 꼬이고 디버깅이 힘들다. 그러므로 ★절대로★ 변경되는 변수를 
// 바깥쪽에 쓰지않는다 -> 리액트 철칙
// 2. 단, 변경되지않는 데이터 const 변수나 공유 함수 등은 컴포넌트 바깥에 쓸 수 있다
...
function MainComp() {
  ...
  // [1] 이름 상태관리 변수
  // -> 특징: 1. 변경된 값을 컴포넌트 소멸 전까지 유지됨 
  // 2. 변수값 변경시 리랜더링 됨 
  // 3. 값 변경시 반영은 useEffect 구역에서 확인됨 -> 의미 : 실제돔에 반영할 때 변경 값이 업데이트 되는 시점임 
    ...
    (e) => {
      ...
      switch (txt) {
        ...
        case "이름바꿔 : useState":
          ...
          setStsName(val);
          console.log("useState변경후:", stsName); // -> 이름을 변경해도 이때 값이 변경되는게 아님
          break;
