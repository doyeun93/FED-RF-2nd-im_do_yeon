...
export default function showSubBox() {
  ...
  // 전체 휠 이벤트 막기때문에 서브 컨텐츠박스도 휠이 되지않는다
  // 따라서 휠 이벤트 버블링막기 해야한다 -> stopPropagation()
  ...
  subViewBox.click(function () {
    ...
    // 사용하고자 하는 데이터 이름을 ul 태그의 data-db 속성에 담아 놓고 이것을 읽어온다
    ...
    // 이미지의 개수를 반영한 배열을 임의로 만들고 필요한 경우 이 배열로 map()을 돌려서 코드를 생성한다
    // 우선 빈배열을 만든다
    let iarr = [];
    // 현장포토일때 사용
    if(db=="liveData"){
